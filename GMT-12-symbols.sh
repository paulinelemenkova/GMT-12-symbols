...
# 20. кривая. math angle arc, optionally with one or two arrow heads
# kwargs: угол, координаты XY начальной точки, координаты XY конечной точки
gmt psxy -R -J -Sm1+bc+ea -W1p,red -O -K -P << EOF >> $ps
15 20 4 0 60
EOF
...
gmt psxy -R -J -Sj1c -W2.5p -Ggreen -O -K << EOF >> $ps
35 5 30 2 3
EOF
# Rotated rectangle. Direction degrees counter-clockwise, x and y-dimension in columns 3, 4, and 5.
gmt psxy -R -J -Sj1c -W2.5p,red -O << EOF >> $ps
40 7 330 2 3
EOF
# Step-13. Convert to image file using GhostScript
gmt psconvert vectors.ps -A0.2c -E720 -Tj -Z
